...
static void node_work(int id, int destination, int read_fd, int neighbor_writes[], int neighbor_count)
{
    ...
    for (;;)
    {
        ...
        ssize_t w = write(neighbor_writes[chosen], &ant, sizeof(ant));
        if (w != (ssize_t)sizeof(ant))
        {
            if (errno == EPIPE)
            {
                printf("Ant %d: got lost\n", ant.id);
...
int main(int argc, char** argv)
{
    ...
    while (!stop_flag)
    {
        ...
        ssize_t w = write(pipes[start][1], &ant, sizeof(ant));
        if (w != (ssize_t)sizeof(ant))
        {
            if (errno == EPIPE)
                break; /* start node collapsed */
            break;
        }
